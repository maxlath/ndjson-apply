...
describe('diff', () => {
  it('should output a diff for a sync function', async () => {
    const { stdout } = await exec('./bin/ndjson-apply --diff ./tests/assets/sync_transformer.js < ./tests/assets/sample.ndjson')
    stdout.should.equal(sampleSyncTransformerDiff)
  })

  it('should output a diff for an async function', async () => {
    const { stdout } = await exec('./bin/ndjson-apply --diff ./tests/assets/async_transformer.js < ./tests/assets/sample.ndjson')
