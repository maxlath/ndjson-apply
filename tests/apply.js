...
describe('apply', () => {
  ...
  it('should accept a sync function', async () => {
    const { stdout } = await exec('./bin/ndjson-apply ./tests/assets/sync_transformer.js < ./tests/assets/sample.ndjson')
  ...
  it('should accept an async function', async () => {
    const { stdout } = await exec('./bin/ndjson-apply ./tests/assets/async_transformer.js < ./tests/assets/sample.ndjson')
    const data = stdout.trim().split('\n').map(line => JSON.parse(line))
    data.should.deepEqual([
  ...
  it('should filter-out lines returning empty', async () => {
    const { stdout } = await exec('./bin/ndjson-apply ./tests/assets/even_only_transformer.js < ./tests/assets/sample.ndjson')
    const data = stdout.trim().split('\n').map(line => JSON.parse(line))
    data.should.deepEqual([
      { a: 789, b: 123, total: 912 }
    ])
